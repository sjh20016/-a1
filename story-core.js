...
Story.createEmptyState = function (seed) {
  return {
    ...
    story: {
      ...
      director: {
        phase: 'inactive',         // inactive | voting | active | interlude | campaign_completed
        candidates: [],
        votesByActorId: {},
        ...
        campaign: {
          status: 'inactive', actIndex: 0, maxActs: 3,
          interludeTurnsRemaining: 0,
          clock: { id: 'campaign_crisis', label: '大劫逼近', current: 0, max: 30, stage: 0, status: 'inactive' },
          ending: null,
        },
      },
...
Story.Migration.migrateToV32 = function (s) {
  ...
  // world
  if (!s.world) s.world = { seed: '', name: '', year: 1, worldBible: {}, publicFacts: [], publicRumors: [], activeWorldHooks: [], factions: [], regions: [], ruleState: null, factionHistory: [] };
  if (!s.world.regions) s.world.regions = [];
  if (!s.world.factionHistory) s.world.factionHistory = [];
  if (!s.world.ruleState && s.world.worldBible) s.world.ruleState = Story.WorldRules.createState(s.world.worldBible);
  // story
  if (!s.story) s.story = {};
  const st = s.story;
  ...
  if (!st.narrativeContinuityMemory) st.narrativeContinuityMemory = { observations: [] };
  if (!st.narrativeContinuityMemory.observations) st.narrativeContinuityMemory.observations = [];
  if (!st.recentCanonicalSummaries) st.recentCanonicalSummaries = [];
  if (!st.lastTurnRecordId) st.lastTurnRecordId = null;
  if (!Number.isInteger(st.stateVersion)) st.stateVersion = st.chapterIndex || 0;
  ...
  if (!st.director.campaign) st.director.campaign = {
    status: st.director.phase === 'active' ? 'active' : 'inactive',
    actIndex: (st.director.completedArcs || []).length + (st.director.activeArc ? 1 : 0), maxActs: 3,
    interludeTurnsRemaining: 0,
    clock: { id: 'campaign_crisis', label: '大劫逼近', current: 0, max: 30, stage: 0, status: st.director.phase === 'active' ? 'active' : 'inactive' },
    ending: null,
  };
  if (!st.director.campaign.clock) st.director.campaign.clock = { id: 'campaign_crisis', label: '大劫逼近', current: 0, max: 30, stage: 0, status: 'active' };
  ...
  (s.actors || []).forEach(function (a, i) {
    ...
    if (a.appearance === undefined) a.appearance = '';
    if (a.background === undefined) a.background = '';
    if (a.speechStyle === undefined) a.speechStyle = '';
  ...
  if (!Array.isArray(s.api.lastNarrationWarnings)) s.api.lastNarrationWarnings = [];
  // settings
...
Story._providerByState = typeof WeakMap !== 'undefined' ? new WeakMap() : null;

/** 将 Provider 运行时绑定到单个会话；未绑定的旧入口继续使用 Story.ai facade。 */
Story.bindProviderContext = function (state, context) {
  if (state && context && Story._providerByState) Story._providerByState.set(state, context);
  return context;
};

Story.providerFor = function (state) {
  var context = state && Story._providerByState && Story._providerByState.get(state);
  if (context && context.inheritGlobal && Story.ai) {
    context.enabled = !!Story.ai.enabled;
    context.provider = Story.ai.provider || null;
    context.timeoutMs = Story.ai.timeoutMs || context.timeoutMs || 30000;
    context.providerMeta = Object.assign({}, Story.ai.providerMeta || {});
  }
  return context || Story.ai;
};
...
/* ============================================================
 * §3.2 WorldRules —— 七维世界骰的统一可执行契约
 * ============================================================ */

Story.WorldRules = Story.WorldRules || {};
Story.WorldRules.DIMENSIONS = ['era', 'terrain', 'order', 'heavenlyLaw', 'greatTribulation', 'aberrant', 'storyGravity'];

Story.WorldRules.RESOLVER_TABLE = {
  era: {
    '盛世': { cultivate: 2, social: 1 }, '衰世': { cultivate: -1, investigate: 1 },
    '末法': { cultivate: -2, use_relic: 2 }, '天倾前夜': { battle: -1, flee: 1, investigate: 1 },
  },
  terrain: {
    '山河大陆': { travel: 1, negotiate: 1 }, '群岛海域': { travel: -1, investigate: 1 },
    '空岛海': { travel: -2, observe: 2 }, '妖域边荒': { battle: -1, investigate: 1 },
    '地下灵脉': { travel: -1, cultivate: 1, investigate: 1 },
  },
  order: {
    '宗门割据': { negotiate: -1, social: 1 }, '皇朝统治': { social: -1, deceive: -1, negotiate: 1 },
    '妖族共治': { social: 1, negotiate: 1 }, '散修乱世': { deceive: 1, steal: 1, social: -1 },
  },
  heavenlyLaw: {
    '飞升受阻': { cultivate: -1, investigate: 1 }, '轮回紊乱': { observe: 1, investigate: 1 },
    '契约具现': { negotiate: 2, deceive: -2 }, '法宝有灵': { use_relic: 2, craft: 1 },
  },
  greatTribulation: {
    '魔潮': { battle: -1, flee: -1 }, '天裂': { travel: -1, observe: 1 },
    '古神苏醒': { cultivate: -1, investigate: 1 }, '灵脉枯竭': { cultivate: -2, craft: -1 },
    '皇朝崩塌': { negotiate: -1, deceive: 1, battle: -1 },
  },
  aberrant: {
    '妖族昌盛': { battle: -1, negotiate: 1 }, '鬼修横行': { cultivate: -1, observe: 1 },
    '灵族复苏': { cultivate: 1, investigate: 1 }, '龙裔遗民': { use_relic: 1, investigate: 1 },
  },
  storyGravity: {
    '夺宝': { investigate: 2, use_relic: 1 }, '宗门': { social: 2, negotiate: 1 },
    '战争': { battle: 2, aid: 1 }, '游历': { travel: 2, observe: 1 },
    '复仇': { investigate: 1, battle: 2 }, '经商': { negotiate: 2, craft: 1 },
    '飞升': { cultivate: 2, investigate: 1 },
  },
};

Story.WorldRules.EVENT_TABLE = {
  era: { '盛世': ['abundance', 'sect_gathering'], '衰世': ['scarcity', 'relic_echo'], '末法': ['scarcity', 'sentient_relic'], '天倾前夜': ['disaster', 'deadline'] },
  terrain: { '山河大陆': ['road', 'city'], '群岛海域': ['sea', 'island'], '空岛海': ['skyway', 'fall'], '妖域边荒': ['demon_border', 'wilderness'], '地下灵脉': ['cavern', 'vein'] },
  order: { '宗门割据': ['sect_conflict'], '皇朝统治': ['law', 'tax'], '妖族共治': ['alliance', 'tension'], '散修乱世': ['bandit', 'market'] },
  heavenlyLaw: { '飞升受阻': ['ascension_barrier'], '轮回紊乱': ['memory_error'], '契约具现': ['manifest_pact'], '法宝有灵': ['relic_will'] },
  greatTribulation: { '魔潮': ['demon_tide'], '天裂': ['sky_crack'], '古神苏醒': ['old_god'], '灵脉枯竭': ['vein_dry'], '皇朝崩塌': ['dynasty_fall'] },
  aberrant: { '妖族昌盛': ['demon_craft'], '鬼修横行': ['soul_hazard'], '灵族复苏': ['spirit_oath'], '龙裔遗民': ['dragon_relic'] },
  storyGravity: { '夺宝': ['treasure'], '宗门': ['sect'], '战争': ['war'], '游历': ['wander'], '复仇': ['revenge'], '经商': ['trade'], '飞升': ['ascend'] },
};

Story.WorldRules._value = function (bible, dimension) {
  if (!bible) return '';
  return bible[dimension] || '';
};

Story.WorldRules.contractFor = function (bible, dimension) {
  var value = Story.WorldRules._value(bible, dimension);
  var source = Story.ORIGIN_EFFECT[dimension] && Story.ORIGIN_EFFECT[dimension][value] || {};
  var modifiers = Story.WorldRules.RESOLVER_TABLE[dimension] && Story.WorldRules.RESOLVER_TABLE[dimension][value] || {};
  var events = Story.WorldRules.EVENT_TABLE[dimension] && Story.WorldRules.EVENT_TABLE[dimension][value] || [];
  var weights = {};
  events.forEach(function (tag, index) { weights[tag] = Math.max(1, 3 - index); });
  return {
    id: dimension + ':' + value,
    dimension: dimension,
    value: value,
    resolverModifiers: Story._clone(modifiers),
    eventWeights: weights,
    turnEffect: { pressurePerTurn: 1, matchedActionBonus: 1 },
    explanation: source.rule || (dimension + '：' + value),
  };
};

Story.WorldRules.compile = function (bible) {
  return Story.WorldRules.DIMENSIONS.map(function (dimension) { return Story.WorldRules.contractFor(bible, dimension); });
};

Story.WorldRules.createState = function (bible) {
  var dimensions = {};
  Story.WorldRules.compile(bible).forEach(function (contract) {
    dimensions[contract.dimension] = {
      id: contract.id, value: contract.value, pressure: 0,
      triggerCount: 0, lastTriggeredChapter: 0,
    };
  });
  return { version: '1.0', turn: 0, dimensions: dimensions, recentEffects: [] };
};

Story.WorldRules.resolverModifiers = function (state, ctx) {
  var category = ctx && ctx.category || '';
  return Story.WorldRules.compile(state && state.world && state.world.worldBible).map(function (contract) {
    return {
      ruleId: contract.id,
      dimension: contract.dimension,
      value: contract.value,
      category: category,
      modifier: contract.resolverModifiers[category] || 0,
      explanation: contract.explanation,
    };
  }).filter(function (entry) { return entry.modifier !== 0; });
};

Story.WorldRules.eventWeights = function (state) {
  var result = {};
  Story.WorldRules.compile(state && state.world && state.world.worldBible).forEach(function (contract) {
    Object.keys(contract.eventWeights).forEach(function (tag) {
      result[tag] = (result[tag] || 0) + contract.eventWeights[tag];
    });
  });
  return result;
};

Story.WorldRules.applyToAction = function (state, action) {
  if (!action) return action;
  var entries = Story.WorldRules.resolverModifiers(state, { category: action.category });
  action.worldRuleModifiers = entries;
  var total = entries.reduce(function (sum, entry) { return sum + entry.modifier; }, 0);
  action.worldRuleModifierTotal = total;
  if (action.category === 'cultivate' && total) {
    (action.actorStatusDeltas || []).forEach(function (delta) {
      if (delta.key === 'cultivationProgress') delta.delta = Math.max(1, delta.delta + total * 2);
    });
    (action.gains || []).forEach(function (gain) {
      if (gain.type === 'cultivation') {
        gain.delta = Math.max(1, (gain.delta || 0) + total * 2);
        gain.text = '世界规则修正后，修为' + (total > 0 ? '更快' : '更艰难地') + '精进。';
      }
    });
  }
  if (action.category === 'travel' && total < 0 && action.timePassed) {
    action.timePassed = { value: Math.max(1, (action.timePassed.value || 1) + Math.abs(total)), unit: action.timePassed.unit || '日' };
  }
  if (total >= 3 && action.outcome === 'partial_success') action.outcome = 'success';
  if (total <= -3 && action.outcome === 'success') {
    action.outcome = 'success_with_cost';
    action.costs = (action.costs || []).concat([{ type: 'world_rule', text: '世界法则使这次行动付出额外代价。' }]);
  }
  return action;
};

Story.WorldRules.applyTurnEffects = function (state, envelope) {
  var ruleState = state.world.ruleState || (state.world.ruleState = Story.WorldRules.createState(state.world.worldBible));
  var categories = (envelope && envelope.actions || []).map(function (action) { return action.category; });
  ruleState.turn = (ruleState.turn || 0) + 1;
  var effects = [];
  Story.WorldRules.compile(state.world.worldBible).forEach(function (contract) {
    var slot = ruleState.dimensions[contract.dimension];
    if (!slot) return;
    var matched = categories.some(function (category) { return !!contract.resolverModifiers[category]; });
    slot.pressure += contract.turnEffect.pressurePerTurn + (matched ? contract.turnEffect.matchedActionBonus : 0);
    if (matched) {
      slot.triggerCount += 1;
      slot.lastTriggeredChapter = state.story.chapterIndex || 0;
      effects.push({ ruleId: contract.id, dimension: contract.dimension, value: contract.value, chapterIndex: state.story.chapterIndex || 0 });
    }
  });
  ruleState.recentEffects = (ruleState.recentEffects || []).concat(effects).slice(-20);
  return effects;
};

Story.WorldRules.explain = function (state) {
  var ruleState = state && state.world && state.world.ruleState;
  return Story.WorldRules.compile(state && state.world && state.world.worldBible).map(function (contract) {
    var slot = ruleState && ruleState.dimensions && ruleState.dimensions[contract.dimension] || {};
    return {
      id: contract.id, dimension: contract.dimension, value: contract.value,
      explanation: contract.explanation,
      resolverModifiers: Story._clone(contract.resolverModifiers),
      eventWeights: Story._clone(contract.eventWeights),
      pressure: slot.pressure || 0,
      triggerCount: slot.triggerCount || 0,
    };
  });
};

Story.FactionSystem = Story.FactionSystem || {};
Story.FactionSystem.afterTurn = function (state) {
  var factions = state && state.world && state.world.factions || [];
  if (!factions.length) return null;
  var turn = state.story.chapterIndex || 0;
  var index = Story.hashSeed((state.world.seed || '') + ':faction:' + turn) % factions.length;
  var faction = factions[index];
  var mode = turn % 3;
  var action;
  if (mode === 0) {
    faction.power = Math.min(100, (faction.power || 0) + 1);
    action = { type: 'consolidate', text: faction.name + '巩固了自身力量。', powerDelta: 1 };
  } else if (mode === 1) {
    faction.control = Math.min(10, (faction.control || 0) + 1);
    faction.stance = faction.control >= 3 ? '进取' : '中立';
    action = { type: 'contest', text: faction.name + '扩大了区域影响。', controlDelta: 1 };
  } else {
    faction.resources = Math.min(20, (faction.resources || 0) + 1);
    action = { type: 'gather', text: faction.name + '取得了一批可用资源。', resourceDelta: 1 };
  }
  action.factionId = faction.id;
  action.chapterIndex = turn;
  faction.lastAction = Story._clone(action);
  state.world.factionHistory = (state.world.factionHistory || []).concat([Story._clone(action)]).slice(-30);
  return action;
};
...
Story.CharacterGenesis._GENERIC_STARS = ['破军', '七杀', '贪狼', '天机', '太阴', '廉贞', '武曲', '天府', '巨门', '天相', '天梁', '擎羊'];
...
Story.CharacterGenesis.rollFatePlate = function (seed, actor, worldBible) {
  ...
  // 自定义道途不被强行归类为剑修；未知道途从完整星池确定性取值。
  const stars = Story.CharacterGenesis._DAO_STARS[dao] || Story.CharacterGenesis._GENERIC_STARS;
...
Story.createActorWithoutRelations = function (storyState, setup) {
  ...
  const actor = {
    ...
    appearance: setup.appearance || setup.publicAppearance || '',
    background: setup.background || '',
    speechStyle: setup.speechStyle || '',
...
Story.createSession = async function (config) {
  ...
  Story.state = state;
  if (config.providerContext) Story.bindProviderContext(state, config.providerContext);
  state.world.seed = seed;
  state.world.name = bible.worldName;
  state.world.year = 1;
  state.world.worldBible = bible;
  state.world.ruleState = Story.WorldRules.createState(bible);
  state.world.factionHistory = [];
  state.world.factions = bible.factionPool.map(function (name, i) {
    return { id: 'fac_' + i, name: name, power: 50, stance: '中立', control: 0, resources: 0, goal: i === 0 ? '扩张影响' : (i === 1 ? '守住根基' : '积累资源'), lastAction: null };
  });
...
Story._generateOpening = async function (state) {
  ...
  state.story.pendingResolution = {
    pendingId: 'pending:turn_opening:v' + state.story.stateVersion,
    expectedVersion: state.story.stateVersion,
    turnId: 'turn_opening',
    envelope: envelope,
    actionsByActorId: {},
    botActions: {},
    ...
    resolutionEcho: Story.Narration.buildResolutionEcho(state, envelope),
  };
  ...
  const providerContext = Story.providerFor(state);
  const contractViolation = Story.Narration.validateTurnContractCompleteness(brief.turnContract);
  if (contractViolation && providerContext && providerContext.providerMeta && providerContext.providerMeta.enforcePublishGate) {
    state.story.pendingResolution.lastNarrationError = {
      code: contractViolation.code,
      message: contractViolation.message,
      rawPreview: '',
      details: Story._clone(contractViolation.details || null),
    };
    Story._setTurnPhase(state, 'narration_failed');
    return;
  }
  const narration = providerContext && providerContext.enabled ? await Story.Provider.narrate(state, brief) : null;
  ...
  const coverageViolation = Story.Narration.blockingViolation(state, providerContext && providerContext.providerMeta && /^mock(?:-|$)/.test(providerContext.providerMeta.provider || '') ? null : Story.Narration.validateCoverage(narration, brief), brief);
...
Story._commitPendingResolution = function (state, narration, isOpening) {
  ...
  if (pr.expectedVersion != null && pr.expectedVersion !== s.stateVersion) {
    var staleError = new Error('过期叙事响应：expectedVersion=' + pr.expectedVersion + ' currentVersion=' + s.stateVersion);
    staleError.code = 'STALE_PENDING_TURN';
    throw staleError;
  }
  ...
  if (!isOpening) {
    Story.WorldRules.applyTurnEffects(state, envelope);
  ...
  if (isOpening) {
    ...
    var arcCompletedThisTurn = !!(pr.directorPlan && (pr.directorPlan.arcDeltas || []).some(function (delta) { return delta.op === 'COMPLETE_ARC'; }));
    Story.ArcScheduler.afterTurn(state, { arcCompletedThisTurn: arcCompletedThisTurn });
  }
  // 3. 生成下一轮选项
...
Story.retryNarration = async function (storyState, options) {
  ...
  // 换模型/Provider（可选）
  const providerContext = Story.providerFor(state);
  if (options.modelOverride && providerContext && providerContext.setModel) {
    try { providerContext.setModel(options.modelOverride); } catch (e) {}
  }

  const isOpening = (pr.turnId === 'turn_opening');
  if (!isOpening && Story.Narration.normalizeLegacyTravelTargets(pr.envelope)) {
    try {
      var repairedPreviewState = Story._clone(state);
      pr.sceneAfterPreview = Story.Scene.composeNext(repairedPreviewState, pr.envelope);
      pr.projectedState = Story.Narration.buildProjectedState(state, repairedPreviewState, pr.envelope, pr.sceneAfterPreview);
    } catch (repairPreviewError) {
      pr.sceneAfterPreview = state.story.currentScene;
      pr.projectedState = Story.Narration.buildProjectedState(state, state, pr.envelope, pr.sceneAfterPreview);
    }
  }
  ...
  if (pr.lastNarrationError) {
    brief.protocolCorrection = Story.Narration.buildRetryCorrection(pr.lastNarrationError, brief.qualityPlan);
  }
  const contractViolation = Story.Narration.validateTurnContractCompleteness(brief.turnContract);
  if (contractViolation && providerContext && providerContext.providerMeta && providerContext.providerMeta.enforcePublishGate) {
...
Story.resolveTurn = async function (storyState, actionsByActorId, options) {
  ...
  state.story.pendingResolution = {
    pendingId: 'pending:' + turnId + ':v' + s.stateVersion,
...
Story.regenerateChapter = async function () {
  ...
  s.currentChapter.narrativeWarnings = Story._clone(chapter.narrativeWarnings || []);
...
Story.Intent._resolveTargetV341 = function (state, actorId, text, category) {
  const t = String(text || '');
  ...
  // 普通地点不必预先登记为场景出口；移动裁决会为它建立通用目的地。
  if (category === 'travel') {
    const destination = Story.Intent.extractTravelDestination(t);
...
Story.Intent.resolveTarget = function (state, actorId, text, category) {
  ...
  if (target.type === 'actor') {
    ...
    present = target.virtualDestination === true || Story.Scene.Entity.normalize(scene.exits).some(function (entry) { return entry.id === target.id; });
    reachable = present;
...
/* 从自由移动文本中保留显式目的地，避免把“小镇”等玩家目标降级成“最近出口”。 */
Story.Intent.extractTravelDestination = function (text) {
  var source = String(text || '').trim();
  var match = source.match(/(?:前往|去往|赶往|奔赴|走向|赶到|来到|抵达|到达)([^，。；！？\s]{1,18})/);
  if (!match) match = source.match(/(?:^|[，。；！？\s])去(?!掉|除)([^，。；！？\s]{1,18})/);
  if (!match) return null;
  var name = String(match[1] || '').replace(/(?:吃饭|用饭|进食|找吃的|寻找吃食|投宿|住宿|歇脚|休息|办事|采购|买东西|拜访|打听|看看|游玩|修炼).*$/, '');
  name = name.replace(/^(?:一趟|一下)/, '').replace(/(?:里面|里边|之中)$/, '').trim();
  if (!name || /^(?:这里|此地|当前地点|下一处地点|最近出口)$/.test(name)) return null;
  return {
    type: 'exit',
    id: 'destination_' + (Story.hashSeed(name) >>> 0).toString(36),
    name: name,
    confidence: 0.84,
    matchedText: name,
...
Story.Director._arcFromCandidate = function (state, candidate) {
...
Story.Director.activateArc = function (state, arcId) {
  ...
  var arc = Story.Director._arcFromCandidate(state, candidate);
  d.activeArc = arc;
  d.phase = 'active';
  d.campaign = d.campaign || {};
  d.campaign.status = 'active';
  d.campaign.actIndex = 1;
  d.campaign.maxActs = Math.min(3, Math.max(1, d.candidates.length));
  d.campaign.interludeTurnsRemaining = 0;
  d.campaign.ending = null;
  d.campaign.clock = d.campaign.clock || { id: 'campaign_crisis', label: '大劫逼近', current: 0, max: 30, stage: 0, status: 'active' };
  d.campaign.clock.status = 'active';
  // 未选中的放入 dormantArcs
  d.candidates.forEach(function (c) {
    ...
    d.dormantArcs.push({
      ...
      family: c.family,
      title: c.title,
      publicPitch: c.publicPitch,
      tags: (c.tags || []).slice(),
      status: 'dormant',
      wakeWhen: {
        all: [
          { type: 'arc_completed', arcId: arcId },
          { type: 'interlude_elapsed', turns: 1 },
        ],
      },
...
/* ---------- ArcScheduler：三幕接力、结构化唤醒与全局危机时钟 ---------- */
Story.ArcScheduler = Story.ArcScheduler || {};

Story.ArcScheduler.evaluatePredicate = function (state, predicate) {
  predicate = predicate || {};
  var d = state && state.story && state.story.director;
  var campaign = d && d.campaign || {};
  if (!d) return false;
  if (predicate.all) return predicate.all.every(function (item) { return Story.ArcScheduler.evaluatePredicate(state, item); });
  if (predicate.any) return predicate.any.some(function (item) { return Story.ArcScheduler.evaluatePredicate(state, item); });
  if (predicate.not) return !Story.ArcScheduler.evaluatePredicate(state, predicate.not);
  if (predicate.type === 'arc_completed') {
    return (d.completedArcs || []).some(function (arc) { return !predicate.arcId || arc.arcId === predicate.arcId; });
  }
  if (predicate.type === 'interlude_elapsed') return (campaign.interludeTurnsRemaining || 0) <= 0;
  if (predicate.type === 'chapter_at_least') return (state.story.chapterIndex || 0) >= (predicate.value || 0);
  if (predicate.type === 'thread_stage_at_least') {
    var thread = (state.story.activeThreads || []).find(function (item) { return item.threadId === predicate.threadId; });
    return !!thread && (thread.stage || 0) >= (predicate.value || 0);
  }
  if (predicate.type === 'world_flag') {
    var flags = state.world && state.world.worldBible && state.world.worldBible.flags || {};
    return flags[predicate.flag] === (predicate.value === undefined ? true : predicate.value);
  }
  return false;
};

Story.ArcScheduler._syncSceneThreads = function (state) {
  var scene = state.story.currentScene;
  if (!scene) return;
  scene.activeThreadIds = (state.story.activeThreads || []).filter(function (thread) {
    return thread && (thread.status === 'active' || thread.status === 'dormant');
  }).map(function (thread) { return thread.threadId; });
};

Story.ArcScheduler.closeArcThreads = function (state, arcId, status) {
  (state.story.activeThreads || []).forEach(function (thread) {
    if (!thread || thread.sourceArcId !== arcId) return;
    if (thread.status !== 'active' && thread.status !== 'dormant') return;
    thread.status = status || 'completed';
    if (thread.status === 'completed') thread.stage = Math.max(thread.stage || 0, thread.maxStage || 1);
    thread.lastAdvancedChapter = state.story.chapterIndex || 0;
  });
  Story.ArcScheduler._syncSceneThreads(state);
};

Story.ArcScheduler.completeArc = function (state, arc) {
  var d = state.story.director;
  var campaign = d.campaign;
  if (!(d.completedArcs || []).some(function (item) { return item.arcId === arc.arcId; })) {
    d.completedArcs.push({ arcId: arc.arcId, recipeId: arc.recipeId, title: arc.title, completedAtChapter: state.story.chapterIndex });
  }
  Story.ArcScheduler.closeArcThreads(state, arc.arcId, 'completed');
  if ((d.dormantArcs || []).length) {
    d.phase = 'interlude';
    campaign.status = 'interlude';
    campaign.interludeTurnsRemaining = 1;
  } else {
    d.phase = 'campaign_completed';
    campaign.status = 'completed';
    campaign.interludeTurnsRemaining = 0;
    campaign.ending = { type: 'success', label: '三幕因果已定', completedAtChapter: state.story.chapterIndex };
    if (campaign.clock) campaign.clock.status = 'completed';
  }
};

Story.ArcScheduler._wakeNextArc = function (state) {
  var d = state.story.director;
  var index = (d.dormantArcs || []).findIndex(function (candidate) {
    return !candidate.wakeWhen || Story.ArcScheduler.evaluatePredicate(state, candidate.wakeWhen);
  });
  if (index < 0) return null;
  var candidate = d.dormantArcs.splice(index, 1)[0];
  var arc = Story.Director._arcFromCandidate(state, candidate);
  d.activeArc = arc;
  d.phase = 'active';
  d.campaign.status = 'active';
  d.campaign.actIndex = Math.min(d.campaign.maxActs || 3, (d.campaign.actIndex || 1) + 1);
  d.campaign.interludeTurnsRemaining = 0;
  ...
  Story.ArcScheduler._syncSceneThreads(state);
  d.lastDirectorEvent = {
    arcId: arc.arcId, beatId: arc.beats[0] && arc.beats[0].beatId,
    result: 'arc_woken', chapterIndex: state.story.chapterIndex,
  };
  return arc;
};

Story.ArcScheduler._failCampaignForClock = function (state) {
  var d = state.story.director;
  var campaign = d.campaign;
  if (d.activeArc && d.activeArc.status === 'active') {
    d.activeArc.status = 'failed';
    Story.ArcScheduler.closeArcThreads(state, d.activeArc.arcId, 'failed');
  }
  d.phase = 'campaign_completed';
  campaign.status = 'failed';
  campaign.ending = { type: 'cost', label: '大劫时钟走尽', completedAtChapter: state.story.chapterIndex };
  campaign.clock.status = 'completed';
  d.lastDirectorEvent = {
    arcId: d.activeArc && d.activeArc.arcId || '', beatId: '',
    result: 'campaign_failed', chapterIndex: state.story.chapterIndex,
  };
};

Story.ArcScheduler.afterTurn = function (state, options) {
  options = options || {};
  var d = state && state.story && state.story.director;
  var campaign = d && d.campaign;
  if (!d || !campaign || campaign.status === 'inactive' || d.phase === 'campaign_completed') return null;
  var clock = campaign.clock;
  if (clock && clock.status === 'active') {
    clock.current = Math.min(clock.max || 30, (clock.current || 0) + 1);
    clock.stage = Math.min(4, Math.floor((clock.current / Math.max(1, clock.max || 30)) * 4));
    if (clock.current >= (clock.max || 30)) {
      Story.ArcScheduler._failCampaignForClock(state);
      return null;
    }
  }
  if (d.phase !== 'interlude' || options.arcCompletedThisTurn) return null;
  campaign.interludeTurnsRemaining = Math.max(0, (campaign.interludeTurnsRemaining || 0) - 1);
  if (campaign.interludeTurnsRemaining > 0) return null;
  return Story.ArcScheduler._wakeNextArc(state);
};
...
Story.Director.applyPlan = function (state, plan) {
  ...
  (plan.arcDeltas || []).forEach(function (ad) {
    if (ad.op === 'ADVANCE_BEAT') {
      ...
      if (curBeat) {
        curBeat.progress = Math.min(curBeat.progressTarget || arc.beatProgressTarget || 1, (curBeat.progress || 0) + 1);
      }
      if (!curBeat || curBeat.progress >= (curBeat.progressTarget || arc.beatProgressTarget || 1)) {
      ...
      if (cb) cb.progress = Math.min(cb.progressTarget || arc.beatProgressTarget || 1, (cb.progress || 0) + 1);
      if (!cb || cb.progress >= (cb.progressTarget || arc.beatProgressTarget || 1)) {
        if (cb) cb.status = 'completed';
        arc.status = 'completed';
        Story.ArcScheduler.completeArc(state, arc);
      }
...
Story.Director.getSnapshot = function (state) {
  ...
  return {
    ...
    campaign: d.campaign ? {
      ...
      actIndex: d.campaign.actIndex || 0,
      maxActs: d.campaign.maxActs || 3,
      interludeTurnsRemaining: d.campaign.interludeTurnsRemaining || 0,
      clock: d.campaign.clock ? {
        id: d.campaign.clock.id,
        label: d.campaign.clock.label,
        current: d.campaign.clock.current || 0,
        max: d.campaign.clock.max || 30,
        stage: d.campaign.clock.stage || 0,
        status: d.campaign.clock.status || 'inactive',
      } : null,
      ending: d.campaign.ending ? Story._clone(d.campaign.ending) : null,
    } : null,
    activeArc: arc ? {
      arcId: arc.arcId,
      ...
      currentBeat: beat ? {
        ...
        progressTarget: beat.progressTarget || arc.beatProgressTarget || 1,
      } : null,
      beats: (arc.beats || []).map(function (b) {
        return {
          ...
          progressTarget: b.progressTarget || arc.beatProgressTarget || 1,
        };
      }),
...
Story.Resolver.resolveActorAction = function (state, scene, intent) {
  ...
  return Story.WorldRules.applyToAction(state, result);
};
...
Story.Resolver._resolveTravel = function (state, scene, intent, base, rng) {
  ...
  if (threadId) base.threadEffects.push({ threadId: threadId, op: 'advance', delta: 1 });
  base.publicEffects = [actor.name + '动身前往' + ((intent.targetId && intent.targetId !== 'nearest_exit') ? (intent.targetName || intent.targetId) : '下一处地点') + '。'];
  return base;
};
...
/**
 * 模型返回前即可公开的确定性裁决回响。只含公开行动、目标、结果与代价，
 * 不复制 privateDelta、私密收益或隐藏线程。
 */
Story.Narration.buildResolutionEcho = function (state, envelope) {
  envelope = envelope || {};
  var actions = (envelope.actions || []).map(function (action) {
    var actor = (state.actors || []).find(function (item) { return item.id === action.actorId; });
    return {
      actorId: action.actorId,
      actorName: actor ? actor.name : action.actorId,
      category: action.category,
      actionText: action.rawText || Story.Narration._catLabel(action.category),
      targetId: action.targetId || '',
      targetName: action.targetName || '',
      outcome: action.outcome,
      outcomeLabel: Story.Narration._outcomeLabel(action.outcome),
      costs: (action.costs || []).map(function (cost) { return cost.text; }).filter(Boolean),
      publicEffects: (action.publicEffects || []).slice(),
      worldRuleModifiers: (action.worldRuleModifiers || []).map(function (entry) {
        return { ruleId: entry.ruleId, dimension: entry.dimension, value: entry.value, modifier: entry.modifier };
      }),
    };
  });
  return {
    ...
    readyAt: Date.now(),
    actions: actions,
    interactions: (envelope.interactions || []).map(function (interaction) {
      return {
        interactionId: interaction.interactionId || '',
        type: interaction.type || interaction.interactionType || '',
        actorIds: (interaction.actorIds || []).slice(),
        publicFacts: (interaction.requiredNarrativeFacts || []).slice(),
      };
    }),
  };
};
...
/* 迁移已锁定的旧回合：恢复被 nearest_exit 占位符吞掉的自由移动目的地。 */
Story.Narration.normalizeLegacyTravelTargets = function (envelope) {
  if (!envelope || !Array.isArray(envelope.actions)) return false;
  var replacements = {};
  var changed = false;
  envelope.actions.forEach(function (action) {
    if (!action || action.category !== 'travel') return;
    var genericTarget = action.targetId === 'nearest_exit' || !action.targetName || /^(?:最近出口|下一处地点)$/.test(action.targetName);
    if (!genericTarget) return;
    var destination = Story.Intent.extractTravelDestination(action.rawText || '');
    if (!destination) return;
    replacements[action.targetId || 'nearest_exit'] = destination.id;
    action.targetType = 'exit';
    action.targetId = destination.id;
    action.targetName = destination.name;
    action.targetKnown = true;
    action.targetPresent = true;
    action.targetReachable = true;
    action.targetResolution = { known: true, present: true, reachable: true };
    action.publicEffects = (action.publicEffects || []).map(function (fact) {
      return String(fact).replace(/最近出口|下一处地点/g, destination.name);
    });
    if (action.actionSequence) {
      ['primaryIntent', 'finalIntent'].forEach(function (key) {
        var step = action.actionSequence[key];
        if (!step || step.targetId !== 'nearest_exit') return;
        step.targetType = 'exit'; step.targetId = destination.id; step.targetName = destination.name;
        step.targetKnown = true; step.targetPresent = true; step.targetReachable = true;
      });
      (action.actionSequence.steps || []).forEach(function (step) {
        if (step.targetId !== 'nearest_exit') return;
        step.targetType = 'exit'; step.targetId = destination.id; step.targetName = destination.name;
        step.targetKnown = true; step.targetPresent = true; step.targetReachable = true;
      });
    }
  ...
  if (changed && envelope.movementDecision) {
    var decision = envelope.movementDecision;
    if (replacements[decision.targetId]) decision.targetId = replacements[decision.targetId];
    decision.conflictingTargets = (decision.conflictingTargets || []).map(function (id) { return replacements[id] || id; });
  }
  return changed;
};

Story.Narration.buildActionDetailGroups = function (action) {
  var raw = String(action && action.rawText || '');
  var groups = [];
  if (/(?:吃饭|用饭|进食|找吃的|寻找吃食|吃东西|填饱肚子|果腹)/.test(raw)) {
    groups.push({ kind: 'purpose', label: '吃饭', tokens: ['吃饭', '用饭', '进食', '吃食', '饭菜', '饭食', '填饱肚子', '果腹', '饱餐'] });
  }
  return groups;
};
...
Story.Narration.buildTurnContract = function (state, envelope, directorPlan, sceneAfter) {
  envelope = envelope || {};
  Story.Narration.normalizeLegacyTravelTargets(envelope);
  const isOpening = envelope.turnId === 'turn_0000';
  const scene = sceneAfter || state.story.currentScene || {};
  const mustRenderFacts = [];
  const supportingActionFacts = [];
  (envelope.actions || []).forEach(function (action, index) {
    const actor = state.actors.find(function (candidate) { return candidate.id === action.actorId; });
    if (!actor) return;
    const isHumanAction = Story._isHumanActor(state, actor);
    const targetActor = state.actors.find(function (candidate) { return candidate.id === action.targetId; });
    const selfTarget = action.targetType === 'self' || action.targetId === 'self' || action.targetId === action.actorId;
    // 兼容旧存档中 targetName="self" 的自指行动，避免要求正文出现英文内部 ID。
    const targetName = selfTarget ? actor.name : (action.targetName || (targetActor && targetActor.name) || action.targetId || '');
    const detailGroups = Story.Narration.buildActionDetailGroups(action);
    ...
    detailGroups.forEach(function (group) { requiredMeaning.push(actor.name + '落实行动目的：' + group.label); });
    ...
    const actionFact = {
      ...
      controller: actor.controller || (isHumanAction ? 'human' : 'bot'),
      narrativeRole: isHumanAction ? 'player' : 'companion',
      constraintLevel: 'authority', expressionPolicy: 'semantic_flexible',
    ...
    // 真人行动是发布契约；机器人行动进入同伴行动层，要求有存在感但遗漏只触发一次修复。
    if (isHumanAction) mustRenderFacts.push(actionFact);
    else supportingActionFacts.push(actionFact);
  });
  ...
  const actorWhitelist = state.actors.map(function (actor) {
    ...
    return {
      id: actor.id, name: actor.name, displayName: actor.displayName || actor.name,
      controller: actor.controller || '', identity: actor.identity || '', daoPath: actor.daoPath || '',
      appearance: actor.appearance || '', personalityTags: Story._clone(actor.personalityTags || []),
      background: actor.background || '', speechStyle: actor.speechStyle || '', publicWish: actor.publicWish || '',
      locationId: projected ? projected.locationId : (actor.locationId || scene.locationId),
      presence: projected ? projected.presence : (actor.presence || 'present'),
    };
  });
  ...
  const submittedHumanActorIds = (envelope.actions || []).filter(function (action) {
    var actor = state.actors.find(function (candidate) { return candidate.id === action.actorId; });
    return actor && Story._isHumanActor(state, actor);
  }).map(function (action) { return action.actorId; });
  return {
    contractVersion: '1.2', turnId: envelope.turnId || '', isOpening: isOpening,
    mustRenderFacts: mustRenderFacts, supportingActionFacts: supportingActionFacts,
    ...
    supportingActionFactIds: supportingActionFacts.map(function (fact) { return fact.factId; }),
    ...
    validationPolicy: {
      policyVersion: '2.0',
      authority: ['submitted actions', 'targets', 'resolver outcomes', 'costs', 'player agency', 'world time'],
      continuityMemory: ['known entities', 'known locations', 'previous summaries', 'style history'],
      expression: 'semantic_flexible',
      openWorldDescription: true,
    },
...
Story.Narration.buildQualityPlan = function (state, contract, projectedState) {
  ...
  var supportingFacts = (contract.supportingActionFacts || []).filter(function (fact) { return fact.kind === 'action'; });
  var allActionFacts = facts.concat(supportingFacts);
  ...
  var montage = facts.map(function (fact) { return fact.factId; }).filter(function (id) { return selected.indexOf(id) < 0; })
    .concat(supportingFacts.map(function (fact) { return fact.factId; }));
  ...
  var companionCount = new Set(supportingFacts.map(function (fact) { return fact.actorId; })).size;
  var narrativeActorCount = Math.max(1, Math.min(4, Math.round(humanCount + companionCount * 0.65)));
  var factCount = allActionFacts.length + interactions.length;
  var publishBase = narrativeActorCount <= 1 ? 260 : (narrativeActorCount === 2 ? 420 : (narrativeActorCount === 3 ? 600 : 760));
  var baseMin = narrativeActorCount <= 1 ? 450 : (narrativeActorCount === 2 ? 650 : (narrativeActorCount === 3 ? 850 : 1050));
  var baseIdeal = narrativeActorCount <= 1 ? 650 : (narrativeActorCount === 2 ? 900 : (narrativeActorCount === 3 ? 1150 : 1450));
  var baseMax = narrativeActorCount <= 1 ? 850 : (narrativeActorCount === 2 ? 1200 : (narrativeActorCount === 3 ? 1500 : 1800));
  ...
  if (supportingFacts.length) paragraphPlan.push({ role: 'ensemble', factIds: supportingFacts.map(function (fact) { return fact.factId; }), purpose: '让每名在场机器人同伴至少有一次可见行动、反应或对白；可合并在同一段，不要逐人汇报。' });
  ...
  return {
    ...
    focalActorIds: Array.from(new Set(actorIds.concat(supportingFacts.map(function (fact) { return fact.actorId; })))),
    presencePlan: supportingFacts.map(function (fact) {
      return { actorId: fact.actorId, actorName: fact.actorName, factId: fact.factId, requirement: 'visible_action_or_reaction', mayShareParagraph: true };
    }),
...
Story.Narration.buildNarrativePacket = function (state, contract, qualityPlan, projectedState) {
  ...
  var serializeActionFact = function (fact) {
    return {
      factId: fact.factId,
      ...
      detailGroups: Story._clone(fact.detailGroups || []),
  ...
  var facts = (contract.mustRenderFacts || []).filter(function (fact) { return fact.kind === 'action'; }).map(serializeActionFact);
  var supportingActions = (contract.supportingActionFacts || []).filter(function (fact) { return fact.kind === 'action'; }).map(serializeActionFact);
  ...
  return {
    ...
    presencePlan: Story._clone(qualityPlan.presencePlan || []),
    ...
    interactions: interactions,
    validationPolicy: Story._clone(contract.validationPolicy || {}),
...
Story.Narration.validateTurnContractCompleteness = function (contract) {
  ...
  var isOpening = contract.isOpening === true || contract.turnId === 'turn_0000';
  // 开局由本地 OpeningEnvelope 建立初始局面，不存在“真人提交行动”。
  if (isOpening && submitted.length === 0 && factActors.length === 0) return null;
...
Story.Narration.buildSystemPrompt = function () {
  return [
    ...
    'turnContract.mustRenderFacts 是本回合已经发生的权威事实，每一项真人行动和 mandatory interaction 都应在正文中产生明确因果；自定义行动优先级最高，但允许同义改写、拆分叙述和自然省略。',
    'turnContract.supportingActionFacts 是机器人同伴已经完成的权威行动。每名在场同伴都应获得至少一次可见行动、即时反应或有意义对白；可以让多人共享段落并与主行动交织，不要写成逐人点名的行动报告。',
    'chosenActions 是契约的可读镜像，coverageAnchors 是兼容覆盖锚点；两者都不能覆盖 mustRenderFacts。',
    '不得替换角色、行动目标、裁决结果和权威时间；不得擅自新增会改变状态的重要势力、敌人、持久物品、伤势、线索或奖励。',
    'entityWhitelist、previousTurnSummary、连续性观察和风格历史是续写记忆，不是封闭世界清单；可以自然描写无状态影响的路人、店铺、饮食、天气与环境细节。',
    ...
    'audit 必须尽量列出 coveredFactIds、coveredSupportingFactIds、coveredInteractionIds、mentionedEntities、mentionedLocations、assertedPersistentClaims、openingMode。',
  ].join('\n');
};

Story.Narration.buildRetryCorrection = function (lastError, qualityPlan) {
  lastError = lastError || {};
  qualityPlan = qualityPlan || {};
  var target = qualityPlan.targetLength || {};
  var floor = Number(lastError.details && lastError.details.minimum) || Number(target.publishFloorChars) || 0;
  var ideal = Number(target.idealChars) || Math.max(floor, 900);
  var repair = lastError.message || lastError.code || '重新生成完整章节';
  if (lastError.code === 'NARRATION_TOO_SHORT') {
    repair = '重写完整正文，去除空白后不得少于 ' + floor + ' 字，并尽量写到约 ' + ideal + ' 字；不能只在原文末尾补一句。';
  } else if (lastError.code === 'INSUFFICIENT_PARAGRAPHS') {
    repair = '重写完整正文并满足 NarrativeQualityPlan 的段落数量；每段都要承担动作、反应或后果。';
  }
  return {
    previousErrors: [{ code: lastError.code || 'UNKNOWN', message: lastError.message || repair, details: Story._clone(lastError.details || null) }],
    requiredRepairs: [repair],
    previousOutputPreview: String(lastError.rawPreview || '').slice(0, 800),
    retryKind: 'manual',
  };
};
...
Story.Narration.buildUserPrompt = function (ctx) {
  ...
  var hardFloor = target.publishFloorChars || target.minChars || 260;
  return prefix + '请按以下顺序处理：NarrativePacket → NarrativeQualityPlan → mustRenderFacts → supportingActionFacts/presencePlan → mandatoryInteractions → canonicalScene/actorLocations → consequencePolicy → entity whitelist → previous public summary → style memory/avoid patterns → protocolCorrection。\n' +
    '硬性长度要求：正文去除空白后不得少于 ' + hardFloor + ' 字；本章目标约 ' + (target.idealChars || 900) + ' 字，建议 ' + (plan.desiredParagraphCount || 4) + ' 段。完整覆盖事实后再润色动作、反应与余波，不要把最低字数当成目标。输出前自检 coveredFactIds 和 coveredInteractionIds，但不要在正文展示 ID。\n' +
    '本回合叙事上下文 JSON：\n' + JSON.stringify(ctx);
};
...
Story.Narration._validateActionFactsLegacy = function (narration, contract) {
...
Story.Narration._factActorWindows = function (text, actorName) {
  var source = String(text || '');
  var name = String(actorName || '');
  if (!name) return [];
  var windows = [];
  ...
  while (cursor < source.length) {
    var index = source.indexOf(name, cursor);
    if (index < 0) break;
    windows.push(source.slice(Math.max(0, index - 140), Math.min(source.length, index + name.length + 220)));
    cursor = index + name.length;
  }
  return windows;
};
Story.Narration._hasAffirmedToken = function (text, token) {
  var source = String(text || '');
  var word = String(token || '');
  if (!word) return false;
  var cursor = 0;
  while (cursor < source.length) {
    var index = source.indexOf(word, cursor);
    if (index < 0) return false;
    var prefix = source.slice(Math.max(0, index - 12), index);
    if (!/(未能|没有|并未|未曾|不曾|毫无|仍未|尚未|并不|未得到|未获得)/.test(prefix)) return true;
    cursor = index + word.length;
  }
  return false;
};
Story.Narration._factTargetTokens = function (fact) {
  var name = String(fact && fact.targetName || '').trim();
  if (!name) return [];
  var tokens = [name];
  if (name.indexOf('小镇') >= 0) tokens = tokens.concat(['小镇', '镇上', '镇子', '集镇']);
  if (name.indexOf('宗门') >= 0) tokens = tokens.concat(['宗门', '山门']);
...
Story.Narration._factActionTokens = function (fact) {
  ...
  var words = {
    rest: ['休息', '休养', '睡', '入睡', '小憩', '打盹', '阖眼', '合上眼', '闭目', '休整', '疗伤', '歇息', '调养'],
    wait: ['等待', '观望', '按兵不动', '静候', '守候', '停留'],
    observe: ['观察', '留意', '盯', '打量', '审视', '环顾', '察看'],
    investigate: ['调查', '追查', '搜查', '查看', '寻找', '探查', '查探', '寻访', '翻查'],
    travel: ['前往', '移动', '动身', '离开', '转向', '跑去', '赶往', '来到', '抵达', '到达', '走向', '下山', '入城', '进城', '入镇', '赶路', '上路'],
    social: ['交谈', '询问', '说', '指认', '指控', '搭话', '开口', '问道', '拜访'],
    negotiate: ['谈判', '交易', '条件', '商议', '议价', '交涉', '协商'],
    cultivate: ['修炼', '闭关', '静修', '打坐', '吐纳', '运功', '调息'],
    craft: ['炼制', '锻造', '修复', '制作', '打造', '铸造'],
    battle: ['攻击', '袭击', '出手', '出剑', '攻向', '命中', '挥剑', '动手', '迎击', '劈向'],
    flee: ['逃', '撤', '退避', '脱身', '离去', '避开'],
    steal: ['偷', '窃取', '盗取', '顺走', '拿走', '摸走'],
    aid: ['援护', '帮助', '支援', '相助', '帮忙', '救援'],
    deceive: ['欺骗', '伪装', '说谎', '诬告', '设局', '假扮', '冒充', '隐瞒'],
    use_relic: ['御器', '法宝', '遗物', '共鸣', '催动', '祭出'],
    freeform: ['行动', '尝试', '着手', '开始']
  };
...
Story.Narration.validateActionFacts = function (narration, contract) {
  ...
  for (var i = 0; i < facts.length; i++) {
    ...
    var outcomeWindow = Story.Narration._factActorWindows(text, fact.actorName).join('\n');
    var selfTarget = fact.targetType === 'self' || fact.targetId === 'self' || !!fact.targetId && fact.targetId === fact.actorId || fact.targetName === 'self' || fact.targetName === '自身' || fact.targetName === fact.actorName;
    var genericTarget = fact.targetId === 'nearest_exit' || /^(?:最近出口|下一处地点)$/.test(fact.targetName || '');
    var targetHit = selfTarget || genericTarget || !fact.targetName || Story.Narration._factTargetTokens(fact).some(function (token) { return outcomeWindow.indexOf(token) >= 0; });
    var detailHit = (fact.detailGroups || []).every(function (group) {
      return (group.tokens || []).some(function (token) { return Story.Narration._hasAffirmedToken(outcomeWindow, token); });
    });
    var outcomeWords = {
      great_success: ['大获成功', '完全成功', '额外线索'], success: ['成功', '命中', '得手', '取得'], quiet_success: ['恢复', '缓和', '安稳', '休整', '睡着', '醒来', '醒了', '精神了些', '呼吸渐稳', '呼吸渐渐平稳'], success_with_cost: ['代价', '受伤', '损耗', '暴露'], partial_success: ['未完全', '部分', '打断', '未能全数'], setback: ['失利', '受挫', '未能', '失败', '没有完成'], missed_opportunity: ['错过', '错失', '机会流失', '逾时', '不候', '作废', '已发完', '来不及'], interrupted: ['打断', '中断', '惊扰', '惊醒', '吵醒'], retreat: ['退避', '撤离', '逃离'], stalemate: ['僵持', '制止'], long_action_request: ['尚未开始', '等待同意', '登记请求']
    };
    var positiveOutcome = ['great_success', 'success', 'quiet_success'].indexOf(fact.outcome) >= 0;
    var outcomeHit = (outcomeWords[fact.outcome] || []).concat([fact.outcomeLabel || '']).some(function (word) {
      return word && (positiveOutcome ? Story.Narration._hasAffirmedToken(outcomeWindow, word) : outcomeWindow.indexOf(word) >= 0);
    });
    var gainTokens = Story.Narration._coverageTokens(fact.gains || []);
    var costTokens = Story.Narration._coverageTokens(fact.costs || []);
    var resultHit = gainTokens.some(function (token) { return Story.Narration._hasAffirmedToken(outcomeWindow, token); }) || costTokens.some(function (token) { return token && outcomeWindow.indexOf(token) >= 0; });
    if (fact.actionType === 'battle' && fact.targetType === 'actor') {
      if (actorHit && actionHit && !targetHit) return Story.Narration._violation('WRONG_ACTION_TARGET', fact.actorName + '的攻击目标必须是' + fact.targetName, text, { factId: fact.factId });
    ...
    if (!(actorHit && actionHit && targetHit && detailHit)) {
      return Story.Narration._violation('MISSING_ACTION_FACT', '正文未在' + fact.actorName + '的局部事实窗口完整落实行动：' + fact.actionText, text, { factId: fact.factId, actionType: fact.actionType, targetId: fact.targetId, actorHit: actorHit, actionHit: actionHit, targetHit: targetHit, detailHit: detailHit, missingDetail: detailHit ? '' : (fact.detailGroups || []).map(function (group) { return group.label; }).join('、') });
    }
    if (!(outcomeHit || resultHit || fact.outcome === 'success' && actionHit)) {
      return Story.Narration._violation('WRONG_ACTION_OUTCOME', '正文已写出' + fact.actorName + '的行动，但未落实本地裁决结果：' + (fact.outcomeLabel || fact.outcome), text, { factId: fact.factId, expectedOutcome: fact.outcome });
    }
  }
  return null;
};

Story.Narration.validateSupportingActionFacts = function (narration, contract) {
  var facts = (contract && contract.supportingActionFacts || []).filter(function (fact) { return fact.kind === 'action'; });
  for (var i = 0; i < facts.length; i++) {
    var violation = Story.Narration.validateActionFacts(narration, { mustRenderFacts: [facts[i]] });
    if (!violation) continue;
    if (violation.code === 'MISSING_ACTION_FACT') {
      return Story.Narration._violation(
        'MISSING_BOT_PRESENCE',
        '机器人同伴' + facts[i].actorName + '本回合没有获得可见的行动、反应或对白：' + facts[i].actionText,
        Story.Narration._parsedText(narration).chapter,
        { factId: facts[i].factId, actorId: facts[i].actorId, actorName: facts[i].actorName }
      );
    }
    // 一旦正文主动写到机器人，目标和结果仍属于权威状态，不能被改写。
    return violation;
  }
  return null;
};
...
Story.Narration.validateEntityAudit = function (narration, contract) {
  ...
  var register = function (value) {
    ...
    whitelist[Story.canonicalEntityId(normalized)] = true;
  };
  (contract.entityWhitelist || []).concat(contract.actorWhitelist || []).forEach(function (entry) {
    if (!entry || typeof entry !== 'object') return;
    register(entry.id);
    register(entry.name);
    register(entry.displayName);
  });
  var mentionedEntities = Array.isArray(parsed.audit.mentionedEntities) ? parsed.audit.mentionedEntities : [];
  var unknown = mentionedEntities.map(function (entry) {
    if (entry && typeof entry === 'object') return entry.id || entry.name || entry.displayName || '';
    return entry;
  }).find(function (identity) {
    if (typeof identity !== 'string' || !identity.trim()) return false;
    var normalized = identity.trim();
    return !whitelist[normalized] && !whitelist[Story.canonicalEntityId(normalized)];
  });
  ...
  var coveredFactIds = Array.isArray(parsed.audit.coveredFactIds) ? parsed.audit.coveredFactIds : [];
  var invalidFact = coveredFactIds.find(function (id) { return ids.indexOf(id) < 0; });
...
Story.Narration.ValidationPolicy = {
  policyVersion: '2.0',
  level: function (violation) {
    var code = violation && violation.code || 'UNKNOWN';
    if (code === 'NARRATION_TOO_SHORT') {
      var actual = Number(violation && violation.details && violation.details.actual) || 0;
      if (actual < 80) return 'block';
      return 'repair';
    }
    var blockers = [
      'NARRATIVE_CONTRACT_INCOMPLETE', 'WRONG_ACTION_TARGET', 'WRONG_ACTION_OUTCOME',
      'PLAYER_AGENCY_VIOLATION', 'UNAUTHORIZED_EVENT', 'UNAUTHORIZED_SUPERNATURAL_EFFECT',
      'UNAUTHORIZED_CLUE', 'UNAUTHORIZED_REWARD', 'FORBIDDEN_REVEAL', 'MODEL_OVERREACH',
      'EVIDENCE_STATUS_VIOLATION', 'TEMPORAL_CONTRADICTION', 'NARRATION_TRUNCATED',
      'TARGET_NOT_PRESENT', 'TARGET_NOT_REACHABLE',
    ];
    if (blockers.indexOf(code) >= 0) return 'block';
    var memoryOnly = [
      'UNREGISTERED_ENTITY', 'UNREGISTERED_LOCATION', 'AUDIT_UNKNOWN_FACT',
      'SCENE_CONTRADICTION', 'CONTINUITY_CONTRADICTION', 'REPETITIVE_OPENING',
      'REPETITIVE_STYLE_PHRASE', 'REPETITIVE_NARRATION',
    ];
    if (memoryOnly.indexOf(code) >= 0) return 'memory';
    return 'repair';
  },
  split: function (violations) {
    var result = { block: [], repair: [], memory: [], all: [] };
    var seen = Object.create(null);
    (violations || []).filter(Boolean).forEach(function (violation) {
      var key = (violation.code || 'UNKNOWN') + '|' + (violation.message || '');
      if (seen[key]) return;
      seen[key] = true;
      var item = Object.assign({}, violation, { policyLevel: Story.Narration.ValidationPolicy.level(violation) });
      result[item.policyLevel].push(item);
...
Story.Narration.collectValidation = function (state, narration, brief, options) {
  options = options || {};
  const contract = brief && brief.turnContract;
  if (!contract) return Story.Narration.ValidationPolicy.split([]);
  const recent = (state && state.story && state.story.recentNarrations || []).slice(-3);
  var checks = [
    ...
    Story.Narration.validateSupportingActionFacts(narration, contract),
    ...
    Story.Narration.validateContinuityAgainstSummary(narration, contract.previousTurnSummary, contract),
    options.includeCompleteness || brief.enforcePublishGate ? Story.Narration.validateChapterCompleteness(narration, contract, { finishReason: narration && narration.finishReason, qualityPlan: brief.qualityPlan }) : null,
  ];
  checks = checks.concat(Story.Narration.validateStyleSoft(narration, state));
  return Story.Narration.ValidationPolicy.split(checks);
};

Story.Narration.rememberWarnings = function (state, warnings, brief) {
  if (!state || !state.story) return [];
  if (!warnings || !warnings.length) {
    if (state.api) state.api.lastNarrationWarnings = [];
    return [];
  }
  var turnId = brief && brief.turnContract && brief.turnContract.turnId || '';
  var compact = warnings.map(function (warning) {
    return {
      code: warning.code || 'NARRATIVE_MEMORY_NOTE',
      message: warning.message || '',
      level: warning.policyLevel || warning.level || Story.Narration.ValidationPolicy.level(warning),
      turnId: turnId,
      chapterIndex: state.story.chapterIndex || 0,
      observedAt: Date.now(),
    };
  });
  var memory = state.story.narrativeContinuityMemory || { observations: [] };
  var merged = (memory.observations || []).concat(compact);
  var seen = Object.create(null);
  memory.observations = merged.reverse().filter(function (item) {
    var key = item.turnId + '|' + item.code + '|' + item.message;
    if (seen[key]) return false;
    seen[key] = true;
    return true;
  }).reverse().slice(-40);
  state.story.narrativeContinuityMemory = memory;
  state.api.lastNarrationWarnings = compact;
  return compact;
};

Story.Narration.blockingViolation = function (state, violation, brief) {
  if (!violation) return null;
  var level = Story.Narration.ValidationPolicy.level(violation);
  if (level === 'block') return violation;
  Story.Narration.rememberWarnings(state, [Object.assign({}, violation, { policyLevel: level })], brief);
  return null;
};

Story.Narration.validateSemantic = function (state, narration, brief, options) {
  options = options || {};
  var evaluation = Story.Narration.collectValidation(state, narration, brief, { includeCompleteness: !!(brief && brief.enforcePublishGate) });
  if (!options.policyAware) return evaluation.all;
  var attempt = Number(options.attempt) || 0;
  var warnings = evaluation.memory.concat(attempt > 0 ? evaluation.repair : []);
  state.api.lastNarrationWarnings = warnings.map(function (warning) { return { code: warning.code, message: warning.message, level: warning.policyLevel }; });
  if (state.api.lastNarrationObservation) {
    state.api.lastNarrationObservation.validationPolicy = {
      policyVersion: Story.Narration.ValidationPolicy.policyVersion,
      blockCount: evaluation.block.length,
      repairCount: evaluation.repair.length,
      memoryCount: evaluation.memory.length,
      acceptedWarningCount: warnings.length,
    };
  }
  return evaluation.block.concat(attempt === 0 ? evaluation.repair : []);
};

Story.Narration.validatePublishGate = function (state, narration, brief) {
  var contract = brief && brief.turnContract;
  if (!contract) return null;
  var completeness = Story.Narration.validateTurnContractCompleteness(contract);
  if (completeness) return completeness;
  var evaluation = Story.Narration.collectValidation(state, narration, brief, { includeCompleteness: true });
  var policyApplied = !!(narration && narration._validationPolicyApplied);
  var warnings = evaluation.memory.concat(policyApplied ? evaluation.repair : []);
  Story.Narration.rememberWarnings(state, warnings, brief);
  var blockers = evaluation.block.concat(policyApplied ? [] : evaluation.repair);
...
Story.Narration.buildPublicContext = function (state, brief, projectedState, qualityPlan) {
  ...
  (contract.supportingActionFacts || []).forEach(function (fact) {
    delete fact.privateGains;
  });
  ...
  var allPublicActionFacts = (contract.mustRenderFacts || []).filter(function (fact) { return fact.kind === 'action'; })
    .concat((contract.supportingActionFacts || []).filter(function (fact) { return fact.kind === 'action'; }));
  var chosenActions = allPublicActionFacts.map(function (fact) {
    return {
      ...
      tags: [fact.actionType], isCustom: fact.source === 'custom', controller: fact.controller || '', narrativeRole: fact.narrativeRole || 'player',
    };
  });
  var turnEvents = [];
  ...
  var publicBrief = {
    ...
    focalActors: Story._clone(brief.focalActors || []), narrationBeats: (contract.mustRenderFacts || []).concat(contract.supportingActionFacts || []).reduce(function (out, fact) {
  ...
  var cast = (state.actors || []).map(function (actor) {
    ...
    return {
      id: actor.id, name: actor.name, controller: actor.controller || '',
      publicProfile: actor.publicProfile || [actor.identity, actor.daoPath].filter(Boolean).join('·'),
      identity: actor.identity || '', daoPath: actor.daoPath || '', appearance: actor.appearance || '',
      personalityTags: Story._clone(actor.personalityTags || []), background: actor.background || '', speechStyle: actor.speechStyle || '',
      currentStatus: actor.statusSummary || '', publicGoal: actor.publicWish || '',
      visibleRelations: Object.keys(relations).length ? relations : Story._clone(actor.relationHints || {}),
    };
  });
  return {
    roomId: state.roomId || '',
    ...
    protocolCorrection: brief.protocolCorrection ? Story._clone(brief.protocolCorrection) : null,
    ...
    continuityMemory: ((state.story.narrativeContinuityMemory || {}).observations || []).slice(-8).map(function (item) { return { code: item.code, message: item.message, level: item.level }; }),
    ...
    constraints: {
      proseLength: publicBrief.style.proseLength,
      paragraphCount: publicBrief.style.paragraphCount,
      authorityBoundary: '行动目标、Resolver 结果、代价、玩家自主权和时间推进不可改写。',
      continuityMemoryIsGuidance: true,
      ensemblePresence: '每名有 supportingActionFacts 的机器人同伴至少获得一次可见行动、即时反应或有意义对白；允许共享段落，不要求机械轮流点名。',
      openWorldDescription: '可添加不改变状态的路人、店铺、饮食、天气和环境细节；重要敌人、奖励、伤势、线索与持久资产必须来自权威事实。',
      noUnjustifiedPowerJump: true,
      narrationOnly: '只返回 {title, chapter, dialogues, endingImage, audit}；不得返回 statePatch、choices、newFacts、newRumors、newHooks、newRelics、changedRelations、timePassed。',
    },
  };
};
...
Story.Narration.assembleFromAI = function (state, scene, envelope, narration) {
  ...
  const narrativeWarnings = narration && Array.isArray(narration._validationWarnings) ? Story._clone(narration._validationWarnings) : [];
  ...
  return {
    ...
    narrativeWarnings: narrativeWarnings,
  };
};
...
Story.Narration.failPendingForDirectorViolation = function (state, violation, countRetry) {
  ...
  state.story.pendingResolution.lastNarrationError = {
    ...
    details: Story._clone(violation.details || null),
  };
...
Story.Narration.applyNarration = function (state, chapter, envelope, isOpening) {
  ...
  s.currentChapter = {
    ...
    narrativeWarnings: Story._clone(chapter.narrativeWarnings || []),
    renderVersion: 0,
  };
...
Story.Provider._withTimeout = function (providerContext, promise) {
  ...
  return new Promise(function (resolve) {
    ...
    Promise.resolve(promise).then(function (value) {
      ...
      if (!done) { done = true; clearTimeout(timer); resolve({ __providerError: error }); }
    });
  });
};
...
Story.Provider.narrate = async function (state, brief) {
  const providerContext = Story.providerFor(state) || {};
  const setStatus = function (status, details) { return Story.setAIStatusFor(state, status, details); };
  if (!providerContext.enabled) {
    setStatus('disabled', { code: 'disabled', message: 'AI 未启用，本回合裁决已保留，等待重试。' });
    state.api.lastStatus = 'offline'; state.api.lastErrorCode = null;
    return null;
  }
  if (!providerContext.provider || typeof providerContext.provider.narrate !== 'function') {
    setStatus('fallback', { code: 'NO_API_CONFIG', message: 'AI 已开启，但缺少可用 Provider，本回合裁决已保留，等待重试。' });
    state.api.lastStatus = 'offline'; state.api.lastErrorCode = 'NO_API_CONFIG';
    return null;
  }
  const requestedAt = Date.now();
  state.api.lastRequestAt = requestedAt;
  setStatus('requesting', { message: '正在请求 AI 叙事……', lastRequestAt: requestedAt });
  try {
    brief.enforcePublishGate = !!(providerContext.providerMeta && providerContext.providerMeta.enforcePublishGate);
    const ctx = Story.Provider._briefToCtx(state, brief);
    const providerName = providerContext.providerMeta && providerContext.providerMeta.provider || '';
    // 无 meta 的 Provider 是 V3.3 旧插件/单元测试兼容路径；浏览器与服务端正式 Provider 都必须提供名称并执行语义校验。
    const skipSemanticValidation = !providerName || /^mock(?:-|$)/.test(providerName) || providerContext.providerMeta && providerContext.providerMeta.skipSemanticValidation;
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      const wrapped = await Story.Provider._withTimeout(providerContext, providerContext.provider.narrate(ctx));
      if (wrapped && wrapped.__providerError) throw wrapped.__providerError;
      // 超时：已等待完整超时窗口，不重试，直接降级
      if (wrapped && wrapped.__timeout) {
        setStatus('fallback', { code: 'TIMEOUT', message: 'API 请求超时，本回合裁决已保留，等待重试。', lastRequestAt: requestedAt });
      ...
      if (!raw) {
        if (attempt === 0) { setStatus('received', { message: '首次响应为空，自动重试一次。', lastRequestAt: requestedAt }); continue; }
        setStatus('fallback', { code: 'EMPTY_RESPONSE', message: 'API 返回空内容，本回合裁决已保留，等待重试。', lastRequestAt: requestedAt });
        state.api.lastStatus = 'offline'; state.api.lastErrorCode = 'EMPTY_RESPONSE';
        return null;
      }
      setStatus('received', { message: attempt === 0 ? '已收到 API 响应，正在校验。' : '已收到重试响应，正在校验。', lastRequestAt: requestedAt });
      ...
      if (errors.length) {
        ...
        if (attempt === 0) {
          ...
          setStatus('received', { message: '检测到格式偏差，自动修复格式后重试。', errors: errors, lastRequestAt: requestedAt }); continue;
        }
        setStatus('fallback', { code: 'INVALID_SCHEMA', message: 'AI 返回字段不合规，本回合裁决已保留，等待重试。', errors: errors, lastRequestAt: requestedAt });
      ...
      const semanticErrors = skipSemanticValidation ? [] : Story.Narration.validateSemantic(state, parsed, brief, { policyAware: true, attempt: attempt });
      if (semanticErrors.length) {
        ...
        if (attempt === 0) {
          ...
          await Story.Narration.emitDebug({ stage: 'correction', roomId: state.roomId || 'local', turnId: brief.turnContract && brief.turnContract.turnId || 'turn_unknown', correction: ctx.protocolCorrection });
          setStatus('received', { message: '章节语义与本地事实契约不符，正在自动修复一次。', errors: semanticErrors, lastRequestAt: requestedAt });
          continue;
        }
        ...
        setStatus('fallback', { code: violation.code, message: violation.message, errors: semanticErrors, lastRequestAt: requestedAt });
        state.api.lastStatus = 'offline'; state.api.lastErrorCode = violation.code; state.api.lastErrorMessage = violation.message;
        return null;
      }
      const acceptedWarnings = skipSemanticValidation ? [] : (state.api.lastNarrationWarnings || []).slice();
      Story.Narration.rememberWarnings(state, acceptedWarnings, brief);
      await Story.Narration.emitDebug({ stage: 'validation', roomId: state.roomId || 'local', turnId: brief.turnContract && brief.turnContract.turnId || 'turn_unknown', attempt: attempt + 1, valid: true, errors: [], warnings: acceptedWarnings });
      // 成功：越权字段在此被剥离（仅保留文案）
      ...
      const successNotes = [];
      if (parsed._autoFixed) successNotes.push('已自动修复格式');
      successNotes.push('已采用 API 正文');
      if (acceptedWarnings.length) successNotes.push('连续性提示已记入叙事记忆');
      const msg = successNotes.join('；') + '。';
      setStatus('success', { message: msg, lastRequestAt: requestedAt });
      return { title: parsed.title, chapter: parsed.chapter, dialogues: parsed.dialogues, endingImage: parsed.endingImage, audit: parsed.audit || null, finishReason: parsed.finishReason || '', _autoFixed: !!parsed._autoFixed, _validationPolicyApplied: !skipSemanticValidation, _validationWarnings: acceptedWarnings };
    }
    return null;
  } catch (e) {
    const code = e && e.code || Story.Provider._classifyError(e && e.message);
    setStatus('fallback', { code: code, message: 'AI 文本生成失败，本回合裁决已保留，等待重试。' + (e && e.message ? '原因：' + e.message : ''), error: e && e.message, lastRequestAt: requestedAt });
...
Story.Provider._classifyError = function (msg) {
  msg = String(msg || '');
  if (/AI_QUOTA_EXCEEDED/i.test(msg)) return 'AI_QUOTA_EXCEEDED';
  if (/AI_CONCURRENCY_LIMIT/i.test(msg)) return 'AI_CONCURRENCY_LIMIT';
...
Story.Provider._briefToCtxLegacyV332 = function (state, brief) {
...
Story.Provider._parseNarrationResponseV341 = function (rawText) {
  let autoFixed = false;
...
Story._setAIStatusOn = function (providerContext, state, details) {
  providerContext = providerContext || Story.ai;
  details = details || {};
  const previous = providerContext.status || {};
  providerContext.status = {
    state: state, code: details.code || state, message: details.message || '', error: details.error || '',
    provider: providerContext.providerMeta && providerContext.providerMeta.provider || '', model: providerContext.providerMeta && providerContext.providerMeta.model || '', base: providerContext.providerMeta && providerContext.providerMeta.base || '',
  ...
  (providerContext._listeners || []).slice().forEach(function (listener) { try { listener(Story._clone(providerContext.status)); } catch (e) {} });
  return providerContext.status;
};

Story.setAIStatus = function (state, details) { return Story._setAIStatusOn(Story.ai, state, details); };
Story.setAIStatusFor = function (storyState, state, details) { return Story._setAIStatusOn(Story.providerFor(storyState), state, details); };
Story.getAIStatus = function (storyState) {
  var providerContext = storyState ? Story.providerFor(storyState) : Story.ai;
  return Story._clone(providerContext && providerContext.status || Story.ai.status);
};
...
Story.getPublicStoryView = function (storyState) {
  ...
  const pending = storyState.story.pendingResolution;
  return {
    ...
    resolutionEcho: pending && pending.resolutionEcho && pending.resolutionEcho.actions && pending.resolutionEcho.actions.length
      ? Story._clone(pending.resolutionEcho) : null,
    year: storyState.world.year,
    worldName: storyState.world.name,
    worldBible: Story._clone(storyState.world.worldBible),
    worldRules: Story.WorldRules.explain(storyState),
    worldEventWeights: Story.WorldRules.eventWeights(storyState),
    ...
    factionHistory: (storyState.world.factionHistory || []).slice(-8),
