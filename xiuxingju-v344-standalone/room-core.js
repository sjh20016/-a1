...
(function () {
  ...
  Room.normalizeBotActorTemplate = function (template, fallbackId) {
    template = template || {};
    var clip = function (value, max) { return String(value || '').trim().slice(0, max); };
    var tags = Array.isArray(template.personalityTags) ? template.personalityTags : String(template.personalityTags || '').split(/[、,，\s]+/);
    tags = tags.map(function (tag) { return clip(tag, 20); }).filter(Boolean).slice(0, 8);
    var name = clip(template.name, 30) || '无名同伴';
    var daoPath = clip(template.daoPath, 40) || '自定义道途';
    return {
      id: clip(template.id, 50) || fallbackId || ('bot_' + Date.now().toString(36)),
      name: name,
      identity: clip(template.identity, 80),
      daoPath: daoPath,
      personalityTags: tags.length ? tags : ['自主', '观察'],
      appearance: clip(template.appearance || template.publicAppearance, 160),
      background: clip(template.background, 300),
      speechStyle: clip(template.speechStyle, 100),
      publicProfile: clip(template.publicProfile, 220),
      publicWish: clip(template.publicWish, 200),
      hiddenFate: clip(template.hiddenFate || template.privateFate, 240),
      startingRelation: Story._clone(template.startingRelation || template.relationHints || {}),
      choicePrefs: template.choicePrefs ? Story._clone(template.choicePrefs) : null,
      agentArc: template.agentArc ? Story._clone(template.agentArc) : { currentGoal: clip(template.publicWish, 200), stage: 0, pressure: 0, milestones: [] },
    };
  };
  ...
  Room.coordinator = {
    ...
    addBotSeat: function (roomId, seatIndex, botConfig) {
      ...
      var actorTemplate = null;
      if (botConfig.actorTemplate) {
        actorTemplate = botConfig.actorTemplate;
      } else if (botConfig.presetIndex != null && Room.PRESET_BOT_ACTORS[botConfig.presetIndex]) {
        actorTemplate = Room.PRESET_BOT_ACTORS[botConfig.presetIndex];
      } else {
        // 无指定时取第一个未占用的预设
        ...
        actorTemplate = tpl || { name: '云游同伴', identity: '来历未明的同行者', daoPath: '自定义道途', personalityTags: ['自主', '观察'] };
      }
      seat.actorSetup = Room.normalizeBotActorTemplate(actorTemplate, 'bot_custom_' + seat.seatId);
      seat.displayName = botConfig.displayName || seat.actorSetup.name || Room.BOT_PROFILES[profileId].label;
    ...
    /** 大厅中修改机器人的策略和完整立命，不限制为内置道途。 */
    configureBotSeat: function (roomId, seatId, botConfig) {
      var room = _requireRoom(roomId);
      if (room.status !== 'lobby') throw new Error('房间已开始，无法修改机器人');
      var seat = room.seats.find(function (candidate) { return candidate.seatId === seatId; });
      if (!seat || seat.kind !== 'bot') throw new Error('只能修改机器人席位');
      botConfig = botConfig || {};
      var profileId = botConfig.profileId || seat.controller.botProfileId || 'pursuer';
      if (!Room.BOT_PROFILES[profileId]) throw new Error('未知机器人策略：' + profileId);
      seat.controller.kind = 'bot-weighted';
      seat.controller.botProfileId = profileId;
      seat.actorSetup = Room.normalizeBotActorTemplate(botConfig.actorTemplate || seat.actorSetup, (seat.actorSetup && seat.actorSetup.id) || ('bot_custom_' + seat.seatId));
      seat.displayName = botConfig.displayName || seat.actorSetup.name || Room.BOT_PROFILES[profileId].label;
      seat.ready = true;
      _logEvent(room, { type: 'BOT_CONFIGURED', visibility: 'public', payload: { seatId: seat.seatId, displayName: seat.displayName, profileId: profileId } });
      return seat;
    },
  ...
  Room.COMMAND_HANDLERS = {
    ...
    configureBotSeat:  function (c) { return Room.coordinator.configureBotSeat(c.roomId, c.seatId, c.botConfig); },
