...
class RoomRuntime {
  ...
  async persist(room) {
    if (this.store) {
      ...
      room.lastActivityAt = room.lastActivityAt || room.updatedAt;
      await this.store.saveRoom(room);
    }
  }

  async persistAllRooms() {
    var rooms = this.Room && this.Room.coordinator && this.Room.coordinator._rooms;
    if (!rooms) return;
    for (var room of rooms.values()) await this.persist(room);
  }

  async cleanupExpiredRooms(now) {
    now = now || Date.now();
    var rooms = this.Room && this.Room.coordinator && this.Room.coordinator._rooms;
    if (!rooms) return 0;
    var removed = 0;
    var ttlMs = (Number(this.config.roomTtlMinutes) || 180) * 60000;
    var idleMs = (Number(this.config.roomIdleMinutes) || 30) * 60000;
    var endedMs = (Number(this.config.roomEndedTtlMinutes) || 60) * 60000;
    var self = this;
    for (var room of Array.from(rooms.values())) {
      var sockets = this.socketsByRoom.get(room.roomId);
      if (sockets && sockets.size) continue;
      var createdAt = Number(room.createdAt) || now;
      var lastActivityAt = Number(room.lastActivityAt || room.updatedAt || createdAt);
      var expired = now - createdAt >= ttlMs || now - lastActivityAt >= idleMs;
      if (room.status === 'ended') expired = now - lastActivityAt >= endedMs;
      if (!expired) continue;
      rooms.delete(room.roomId);
      if (room.roomCode) self.roomCodes.delete(room.roomCode);
      if (self.store) {
        try { await self.store.deleteRoom(room.roomId); } catch (error) { self.logger.warn('room cleanup failed'); }
      }
      removed++;
    }
    return removed;
  ...
  restoreRuntimeState(room) {
    ...
    room.lastActivityAt = room.lastActivityAt || room.updatedAt || room.createdAt || Date.now();
  ...
  async createRoom(socket, payload) {
    ...
    var room = this.Room.coordinator.createRoom({ hostName: hostName, mode: 'online', maxSeats: this.config.maxSeats, seed: payload.seed || '', narrativeProfile: profile });
    ...
    return this.withRoomLock(room.roomId, async function () {
      ...
      try { switch (message.type) {
        ...
        case 'finalize_arc_vote':
          ...
          break;
        case 'submit_action': {
          if (Number(self.config.maxRounds) > 0 && room.turn && room.turn.round > Number(self.config.maxRounds)) {
            throw new ProtocolError('ROUND_LIMIT_REACHED', '本局测试回合已结束');
          }
          ...
          if (Number(self.config.maxRounds) > 0 && room.turn && room.turn.round > Number(self.config.maxRounds)) {
            room.status = 'ended';
            room.turn.phase = 'ended';
            if (room.storySession && room.storySession.story) room.storySession.story.turnPhase = 'ended';
          }
          break;
        }
        case 'cancel_action':
