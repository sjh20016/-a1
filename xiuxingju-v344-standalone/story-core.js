...
Story._generateOpening = async function (state) {
  ...
  const coverageViolation = Story.Narration.blockingViolation(state, Story.ai.providerMeta && /^mock(?:-|$)/.test(Story.ai.providerMeta.provider || '') ? null : Story.Narration.validateCoverage(narration, brief), brief);
